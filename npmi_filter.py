...
def get_prob_unrated_movie_user(count_and_list_of_user_rated_movie,total_users,unrated_movie):
	...
	for movie in unrated_movie:
		prob_unrated_movie_user[movie] = count_and_list_of_user_rated_movie[movie][0]/total_users
...
def get_prob_user_watched_both(starting_time,count_and_list_of_user_rated_movie,total_users,rated_movie,unrated_movie):
	...
	for unrat_movie in unrated_movie:
		...
		prob_user_watched_both[unrat_movie] = prob_watched_both
...
def npmi_filter(prob_watched_rated_movie,prob_watched_unrated_movie_user,prob_user_watched_both):
	...
	for movie in prob_user_watched_both:
		npmi = 0
		for rat_movie_index in range(0,len(prob_user_watched_both[movie])):
			pmi = prob_user_watched_both[movie][rat_movie_index]/(prob_watched_rated_movie[rat_movie_index]*prob_watched_unrated_movie_user[movie])
			...
			if(pmi==0):
				...
				npmi += math.log(pmi,2)/(-math.log(prob_user_watched_both[movie][rat_movie_index],2))
		normalised_pointwise_mutual_info[movie] = npmi/len(prob_user_watched_both[movie])
